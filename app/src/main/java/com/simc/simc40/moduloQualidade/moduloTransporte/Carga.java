...
public class Carga extends AppCompatActivity implements SharedPrefsExceptionErrorList, LayoutExceptionErrorList, Etapas, QualidadeExceptionErrorList, ListaLeitores, FirebasePecaPaths, FirebaseGalpaoPaths, FirebaseRomaneioPaths, ReaderExceptionErrorList {
    ...
    TextView goBack, textViewObra, textViewPeca, textViewGalpao, textViewMotorista, textViewVeiculo;
    DatabaseReference reference;
    static final String etapa = cargaKey;
    HashMap<String, Object> firebaseObj, controle_galpao, carga_romaneio;
    Button read, clear;
...
//            serviceRfidReader.configureReaderLayout(Carga.this, tagItem, readCallback, clearCallback);
//        }
//    };
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        textViewGalpao = findViewById(R.id.galpao);
        ...
        intent = getIntent();
        romaneio = (Romaneio) intent.getSerializableExtra("dependency");

        textViewObra.setText(romaneio.getObra().getNome_obra());
        textViewObra.setTag(romaneio.getObra());
        textViewMotorista.setText(romaneio.getMotorista().getNome());
        textViewVeiculo.setText(romaneio.getVeiculo().getNome());
        ...
        readCallback = tag -> {
            try{
                ...
                if(!romaneio.getPecas().contains(peca)) throw new ReaderException(EXCEPTION_TAG_NOT_REGISTED_IN_ROMANEIO);

                serviceRfidReader.addTagToLayout(peca);
                textViewPeca.setText(peca.getNome_peca());
                textViewGalpao.setText(peca.getGalpaoObject().getNome());
                textViewGalpao.setTag(peca.getGalpaoObject());
...
//        bindService(sIntent, serviceConnection, 0);
        ...
        submitForm.setOnClickListener(view -> {
            ...
            try{
                ...
                Galpao galpao = (Galpao) textViewGalpao.getTag();
                controle_galpao = new HashMap<>();
                controle_galpao.put(firebaseGalpaoPathControlSaida, data);

                carga_romaneio = new HashMap<>();
                carga_romaneio.put(firebaseRomaneioPathDataCarga, data);
                ...
                reference.child(firebasePecaPathsFirstKey).child(peca.getObraObject().getUid()).child(peca.getElementoObject().getUid()).child(peca.getTag()).child(firebasePecaPathsSecondKey).child(etapa).setValue(etapa_atual)
                        ...
                        .addOnFailureListener(e -> ErrorHandling.handleError(this.getClass().getSimpleName(), e, modalDeCarregamento, modalAlertaDeErro));
                reference.child(firebaseGalpaoPathFirstKey).child(galpao.getUid()).child(firebaseGalpaoPathControl).child(peca.getTag()).child(firebasePecaPathsSecondKey).updateChildren(controle_galpao)
                        .addOnSuccessListener(unused -> {
                            modalDeCarregamento.avancarPasso(); // 2.4
                            checkPostResponse();
                        })
                        .addOnFailureListener(e -> ErrorHandling.handleError(this.getClass().getSimpleName(), e, modalDeCarregamento, modalAlertaDeErro));
                if(romaneio
                .getPecas()
                .stream()
                .map(Peca::getEtapa_atual)
                .map(etapasDePeca::indexOf)
                .filter(x -> x <= etapasDePeca.indexOf(cargaKey))
                .count() > 1){
                    modalDeCarregamento.avancarPasso(); // 2.5
                    checkPostResponse();
                }else{
        ...
        new ApiChecklist(obra.getUid(), this, database, this.getClass().getSimpleName(), modalDeCarregamento, modalAlertaDeErro, apiCallback, null);
    }

    void checkPostResponse(){
        postResponse.add(true);
        if(postResponse.size() == 4){
            modalDeCarregamento.passoFinal(); // 2.6
            ...
            successDialog.getButton1().setOnClickListener(view1 -> {
                ...
                setResult (Carga.RESULT_OK, intent);
                finish();
            });
        }
    }
