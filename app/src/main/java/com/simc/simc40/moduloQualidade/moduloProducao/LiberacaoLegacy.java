...
public class LiberacaoLegacy extends AppCompatActivity implements SharedPrefsExceptionErrorList, LayoutExceptionErrorList, Etapas, QualidadeExceptionErrorList, ListaLeitores, FirebasePecaPaths , FirebaseGalpaoPaths, ReaderExceptionErrorList {
    ...
    TextView goBack, textViewObra, textViewPeca, textViewGalpao;
    DatabaseReference reference;
    static final String etapa = liberacaoKey;
    HashMap<String, Object> firebaseObj, controle_galpao;
    Button read, clear;
...
//            serviceRfidReader.configureReaderLayout(Liberacao.this, tagItem, readCallback, clearCallback);
//        }
//    };
//
...
//    };
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.mproducao_liberacao);
...
//        bindService(sIntent, serviceConnection, 0);
        ...
        textViewGalpao.setOnClickListener(view -> {
            try {
                Peca peca = serviceRfidReader.getPeca();
                if(peca == null) throw new QualidadeException(EXCEPTION_NO_SELECTED_TAG_ON_SUBMIT);
                if(peca.getTag() == null || peca.getTag().isEmpty()) throw new QualidadeException(EXCEPTION_NO_SELECTED_TAG_ON_SUBMIT);
                Intent gintent = new Intent(this, SelecaoListaGalpoes.class);
                selectGalpaoFromList.launch(gintent);
        ...
        submitForm.setOnClickListener(view -> {
            ...
            try{
                ...
                if(galpao == null) throw new QualidadeException(EXCEPTION_GALPAO_NOT_FILLED);
                ...
                etapa_atual.put(firebasePecaPathsGalpao, galpao.getUid());
                ...
                controle_galpao.put(firebaseGalpaoPathControlEntrada, data);
                controle_galpao.put(firebaseGalpaoPathControlObra, peca.getObraObject().getUid());
                ...
                reference.child(firebasePecaPathsFirstKey).child(peca.getObraObject().getUid()).child(peca.getElementoObject().getUid()).child(peca.getTag()).child(firebasePecaPathsSecondKey).child(etapa).setValue(etapa_atual)
                        ...
                        .addOnFailureListener(e -> ErrorHandling.handleError(this.getClass().getSimpleName(), e, modalDeCarregamento, modalAlertaDeErro));
                reference.child(firebaseGalpaoPathFirstKey).child(galpao.getUid()).child(firebaseGalpaoPathControl).child(peca.getTag()).child(firebasePecaPathsSecondKey).setValue(controle_galpao)
                        .addOnSuccessListener(unused -> {
...
//        new ApiChecklist(this, database, this.getClass().getSimpleName(), loadingDialog, errorDialog, apiCallback, null);
    ...
    result -> {
        if (result.getResultCode() == Activity.RESULT_OK) {
            try {
                ...
                textViewGalpao.setText(galpao.getNome());
            } catch (LayoutException e) {
                modalAlertaDeErro.getBotao().setOnClickListener(view -> {
                    modalAlertaDeErro.fecharModal();
                    onBackPressed();
    ...
    ActivityResultLauncher<Intent> reportError = registerForActivityResult(
            new ActivityResultContracts.StartActivityForResult(),
            result -> {
                if (result.getResultCode() == Activity.RESULT_OK) {
                    reestartActivity();
