...
public class CadastroLegacy extends AppCompatActivity implements SharedPrefsExceptionErrorList, LayoutExceptionErrorList, Etapas, QualidadeExceptionErrorList, ListaLeitores, FirebasePecaPaths, FirebaseElementoPaths, ReaderExceptionErrorList {
...
//            serviceRfidReader.configureReaderLayout(CadastroLegacy.this, tagItem, readCallback, clearCallback);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        readCallback = tag -> {
            try {
                ...
                Elemento elemento = (Elemento) textViewElemento.getTag();
                if(ServiceRfidReader.tagRegistered(tag, pecaTreeMap)) throw new ReaderException(EXCEPTION_WRONG_ETAPA + tag + "etapa" + pecaTreeMap.get(tag).getEtapa_atual());
                ...
                if(1 + numPecas > numMax) throw new QualidadeException(EXCEPTION_MAX_POSSIBLE_READ_PECAS_REACHED + elemento.getPecasPlanejadas());
                if(ServiceRfidReader.tagRegistered(tag, pecaTreeMap) && ServiceRfidReader.tagEtapaIncorreta(tag, etapa, pecaTreeMap))throw new ReaderException(EXCEPTION_WRONG_ETAPA + tag + "etapa" + pecaTreeMap.get(tag).getEtapa_atual());
                String uid = "";
                Peca peca = new Peca(uid, tag, "", etapa, obra, elemento, elemento.getNome() + (numPecas + 1));
...
//        bindService(sIntent, serviceConnection, 0);
        ...
        submitForm.setOnClickListener(view -> {
            ...
            modalDeCarregamento.mostrarModalDeCarregamento(5);
            try{
                Obra obra = (Obra) textViewObra.getTag();
                Elemento elemento = (Elemento) textViewElemento.getTag();
                Peca peca = serviceRfidReader.getPeca();
                ...
                firebaseObj.put(firebasePecaPathsObra, obra.getUid());
                firebaseObj.put(firebasePecaPathsElemento, elemento.getUid());
                ...
                int num = Integer.parseInt(elemento.getPecasCadastradas());
                num++;
                ...
                pecaObj.put(firebasePecaPathsNomePeca, peca.getElementoObject().getNome() + "-" + num);
                ...
                reference.child("pecas").child(peca.getObraObject().getUid()).child(peca.getElementoObject().getUid()).child(peca.getTag()).setValue(pecaObj)
                        .addOnSuccessListener(unused -> {
                            modalDeCarregamento.avancarPasso(); // + serviceRfidReader.getTagMap().size() 2.3
                ...
                reference.child("pecas").child(peca.getObraObject().getUid()).child(peca.getElementoObject().getUid()).child(peca.getTag()).child(firebasePecaPathsSecondKey).child(etapa).setValue(etapa_atual)
                        .addOnSuccessListener(unused -> {
                            modalDeCarregamento.avancarPasso(); // + serviceRfidReader.getTagMap().size() 2.4
                            checkPostResponse(finalNum);
                        }).addOnFailureListener(e -> ErrorHandling.handleError(this.getClass().getSimpleName(), e, modalDeCarregamento, modalAlertaDeErro));
            }catch (Exception e){
                ErrorHandling.handleError(this.getClass().getSimpleName(), e, modalDeCarregamento, modalAlertaDeErro);
...
//        new ApiChecklist(this, database, this.getClass().getSimpleName(), loadingDialog, errorDialog, apiCallback, null);
    ...
    void checkPostResponse(int num){
        ...
        if(postResponse.size() == 2){
            reference.child(firebaseElementoPathFirstKey).child(obra.getUid()).child(elemento.getUid()).child(firebaseElementoPathPecasCadastradasKey).setValue(String.valueOf(num))
            .addOnSuccessListener(unused -> {
    ...
    result -> {
        if (result.getResultCode() == Activity.RESULT_OK) {
            try{
                ...
                textViewElemento.setText(elemento.getNome());
                textViewElemento.setTag(elemento);
                serviceRfidReader.clear();
                generateErrorlist();
            }catch (Exception e){
                ErrorHandling.handleError(this.getClass().getSimpleName(), e, modalDeCarregamento, modalAlertaDeErro);
